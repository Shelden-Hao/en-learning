...
export class PayService {
  ...
  async create(createPayDto: CreatePayDto, user: TokenPayload) {
    ...
    const result = await this.prismaService.$transaction(async (tx) => {
      ...
      //2.支付宝SDK发起支付生成url
      const dateTime = dayjs().add(1, 'minute'); //当前的时间增加了一分钟 为了测试时间短些（实际业务一般 15min~30min ）
      const payUrl = this.sharedPayService
        ...
        .pageExecute('alipay.trade.page.pay', 'GET', {
          bizContent: {
            ...
            product_code: 'FAST_INSTANT_TRADE_PAY', //产品编码（固定值）
      ...
      return {
        payUrl, //返回支付宝的支付链接
        timeExpire: dateTime.toDate().getTime(), // ElementPlus 组件要求是时间戳
      };
    });

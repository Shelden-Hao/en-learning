...
export class TrackerService {
  ...
  async pv(body: PvDto) {
    ...
    if (!existingVisitor) {
      // visitorId 不存在，尝试用 anonymousId 兜底重建
      if (body.anonymousId) {
        const visitor = await this.prismaService.visitor.upsert({
          where: { anonymousId: body.anonymousId },
          create: { anonymousId: body.anonymousId },
          update: {},
          select: { id: true },
        });
        visitorId = visitor.id;
      } else {
        // 既没有有效的 visitorId 也没有 anonymousId，无法上报
        return this.responseService.error(null, 'visitor not found');
      }
    }

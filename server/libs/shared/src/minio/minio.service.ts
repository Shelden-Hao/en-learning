...
export class MinioService implements OnModuleInit {
  ...
  // 模块初始化之后自动执行的 hook
  async onModuleInit() {
    // 启动时创建存储桶，如果我们之前有在页面手动创建桶的，桶默认是私有的，无法通过url访问（所以先删掉这个桶：`C:\Develop\minio>mc rb --force myminio/avatar`）
    // 所以我们一般在程序启动时自动创建一个公有存储桶
          ...
          Statement: [
            {
              ...
              Resource: ['arn:aws:s3:::avatar/*'], //允许读取 avatar  桶内的所有资源
            },
          ],
        }),
      );

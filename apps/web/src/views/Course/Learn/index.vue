...
const saveWordMaster = async () => {
  ...
  // 这里实际业务 token 刷新时间不能太短，否则可能因为请求 401 竞争出现 updateUserWordNumber 页面没有更新的问题
  if (res.success) {
...
const onKeyDown = (index: number, event: KeyboardEvent) => {
  ...
  // 按下了一个键盘的某个字符，不是 ctrl、不是 win（windows）、 command（Mac）、不是alt键
  if (
    event.key.length == 1 &&
    !event.ctrlKey &&
    !event.metaKey &&

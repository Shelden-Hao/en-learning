...
import { OrbitControls } from "three/examples/jsm/controls/OrbitControls.js"; // 轨道控制器，模型可以被退拽 缩放 旋转
import { useTemplateRef, onMounted } from "vue";

const hologramRef = useTemplateRef<HTMLCanvasElement>("hologramRef");
const initThree = () => {
  // 创建场景 -> 网格(几何 + 材质)/模型 obj gltf + 相机 + 渲染器 + 灯光(可选的)
  ...
  const animate = () => {
    ...
    const delta = clock.getDelta(); // 获取两帧之间的时间，clock内置时钟解决时间戳精度不够的问题
    if (mixer) {
